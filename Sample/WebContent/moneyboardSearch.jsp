...
<%
	...
	String moneycategory = request.getParameter("moneycategory");
	
/**********************************************************************/	
// 값 넘어오는거 확인용
	System.out.println("--------------------------------------");
	System.out.println("money서치 정보 : " + moneysearch);
	System.out.println("money카테고리 정보 : " + moneycategory);
	System.out.println("--------------------------------------");
/**********************************************************************/
	int count = boardDAO.getMoneySerachCount(moneysearch, moneycategory);

	int pageSize = 5; 	//하나의 페이지 마다 보여줄 글개수 5
	...
	if(count > 0){
		...
		list = boardDAO.getmoneyBoardList(startRow,pageSize,moneysearch, moneycategory); 
																		// 검색인자도 같이넣어 moneydao에서 처리함
																		// getmoneyBoardList를 오버로딩해서 게시글 검색후 보여주는 
																		// 페이지로 사용 
	...
	%>
		...
		<form action="moneyboardSearch.jsp">
            ...
            <input id="MOD_BLOG1_EmailField" type="text" placeholder="검색할 회사를 입력하세요" name="moneysearch">
         	...
         	</div>	<!-- div밖으로 select 넣으면 밑으로 해서 나옴 -->		
         </form>

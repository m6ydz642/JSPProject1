...
// String newscategory = request.getParameter("newscategory");// 어짜피 처음부터   null값 이라 전체 검색결과 가져옴
// String newssearch = request.getParameter("newssearch");
// 위에꺼 안됨  오류남
String newscategory = ""; // 처음부터 일부로 널값을 넣어서 전체 검색결과를 가져옴
String newssearch = "";// 처음부터 일부로 널값을 넣어서 전체 검색결과를 가져옴

//하나의 페이지 마다 보여줄 글개수 3
int pageSize =5;
...
//(현재 보여질 페이지번호  - 1) * 한페이지당 보여줄 글개수 3
...
if (count > 0) {
	...
	list = boardDAO.getNewsBoardList(startRow, pageSize,newscategory,newssearch);
	System.out.println(list.size() + " 헤드라인 뉴스 보드테이블 사이즈 ");
}
%>
...
<!-- 둘이 합쳐서 줄바꾸기 할까 생각중 -->
<jsp:include page="Crolling/cds.jsp" />
<jsp:include page="Crolling/exchanges.jsp" />
		<%
			//만약 게시판 글개수가 조회 된다면
		if (count > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				NewsBoardBean newsbean = list.get(i);
		%>
...
<%-- onclick="location.href='content.jsp?num=<%=bean.getNum()%>&pageNum=<%=pageNum%>'" --%>
<%-- <a href="NewsBoard.jsp?id=<%=bean.getId()%>&pageNum=<%=pageNum%>" --%>
				<header>
					<a href="javascript:return false;" onclick="location.href='NewsContent.jsp?num=<%=newsbean.getNum()%>&pageNum=<%=newsbean.getNum()%>'" 
					class="MOD_BLOGROLL1_SingleTitle" data-theme="_ts2">

					<h2><%=newsbean.getSubject()%></h2>
				</a>
				<div class="MOD_BLOG1_Meta">
					<p class="MOD_BLOG1_Author"><%=newsbean.getUserid()%>
					</p>
					<p>
...
<%-- 					<p><%=bean.getContent()%></p> 내용은 게시판 안에서만 --%>
				</div>
			</div>
